...
@dataclass
class TransformerGNNConfig:
    model_type: str = "roberta" # Options: 'roberta', 'distilbert'
    ...
    def _build_graphs(self):
        ...
        for item in tqdm(self.samples, desc="Initialising Graph Representations"):
            ...
            if self.text_format == "headline":
                combined_model_text = headline
                spacy_text = headline
            elif self.text_format == "headline_section":
                combined_model_text = f"{headline} {sep_token} {section}"
                spacy_text = headline
            elif self.text_format == "all":
                combined_model_text = f"{headline} {sep_token} {section} {sep_token} {description}"
            ...
            if self.use_conceptnet:
                ...
                for i, token in enumerate(tokens):
                    ...
                    # Clean both RoBERTa (Ġ) and DistilBERT (##) subword artifacts dynamically
...
class TransformerGNNModel(nn.Module):
    def __init__(self, cfg: TransformerGNNConfig, num_relations: int, irf_weights: torch.Tensor):
        super().__init__()
        self.cfg = cfg
 
        self.encoder = AutoModel.from_pretrained(cfg.pretrained_name)
    ...
    def forward(self, input_ids, attention_mask, edge_index, edge_attr, edge_weight):
        ...
        edge_type = edge_attr.view(-1)
        
        valid_edge_mask = edge_type < self.rgcn.num_relations
        edge_type = torch.where(valid_edge_mask, edge_type, torch.zeros_like(edge_type))

        x_gnn_flat = self.rgcn(x_flat, edge_index, edge_type)
...
def build_pipeline(
    ...
    cfg: Optional[TransformerGNNConfig] = None,
) -> None:
    cfg = cfg or TransformerGNNConfig()
    ...
    model = TransformerGNNModel(cfg, total_unique_relations, irf_weights)
    history = train(model, train_loader, val_loader, cfg, device)

    # Output Model
    output_path = os.path.join(cfg.output_dir, f"sarcasm_gnn_model_{cfg.model_type}_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.pt")
...
def predict(texts: list[str], model_path: str = "sarcasm_gnn_model.pt", output_dir: str = "results") -> list[str]:
    ...
    model = TransformerGNNModel(cfg, num_relations, irf_weights)
