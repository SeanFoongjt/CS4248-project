...
def fetch_wayback_response(wayback_url: str, retries: int, backoff_factor: float):
...
def process_row(row, retries: int, backoff_factor: float):
...
def process_rows(
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {
            executor.submit(process_row, row, retries, backoff_factor): row["article_link"]
            for row in rows
        }
        try:
            for future in as_completed(futures):
                try:
                    ...
                    processed += 1
                    fetch_errors += 1
                    rate_limit_row = dict(exc.row)
                    rate_limit_row["fetch_error"] = (
                        f"HTTP 429 Too Many Requests: {exc.response_excerpt}"
                    )
                    pending_writes.append(rate_limit_row)
                    log(
                        f"[RATE-LIMIT] processed={processed}/{len(rows)} status={exc.status_code} "
                        f"article_link={exc.row['article_link']} response_excerpt={exc.response_excerpt}"
                    )
    ...
    if rate_limit_hit:
        log(
            f"[DONE] stopped_early_due_to_rate_limit processed={processed} "
            f"fetch_errors={fetch_errors} parse_errors={parse_errors} output={output_jsonl.resolve()}"
        )
        return

    log(
        f"[DONE] processed={processed} fetch_errors={fetch_errors} parse_errors={parse_errors} "
...
def parse_args():
    ...
    parser.add_argument("--save-every-n", type=int, default=100, help="Checkpoint every N processed rows.")
